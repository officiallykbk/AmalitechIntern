...
const addTask = async (title,description,due_date,priority) => {
    try {
        const query = 'INSERT INTO tasks (title, description, due_date, priority) VALUES ($1, $2, $3, $4)'
...
const manipulateTask = async(id,title,description,due_date,priority,completion_status) => {
        try {
        const query = 'UPDATE Tasks set title=$1,description=$2,due_date=$3,priority=$4,completion_status=$5 WHERE id=$6'
...
const changeCompletion = async(id,completion_status) => {
        try {
        const query = 'UPDATE Tasks set completion_status=$1 WHERE id=$2'
